...
import { layout, makeDefaultEntity } from '@/config/forms/shipment/CreditNote';
...
function onPayToChanged(entity, columns, value) {
  console.log('payToChanged')
  const defaultCurrency = {
    currency: entity.currency,
    rate: entity.exchangeRates[entity.currency]
  }

  const alreadyCharges = props.shipment.ebitNotes
    .filter(e => e.type === EbitNoteType.Credit)
    ...
    entity.chargeItems = props.shipment.quote.prices.reduce((result, rawPrice) => {
    if(alreadyCharges.includes(rawPrice.id)) return result
    if(entity.payTo){
      if(rawPrice.provider && entity.payTo.id !== rawPrice.provider.id) return result
      if(!rawPrice.provider) {
        // condition for commission to client
        const isCommissionToClient  = rawPrice.chargeName === 'Commission to Client' && entity.targetType === 'client'
        if(!isCommissionToClient && entity.payTo.id !== 'isEmpty') return result
      }
    }

    const price = JSON.parse(JSON.stringify(rawPrice))
    const chargeItem = priceToChargeItem(price, EbitNoteType.Credit, defaultCurrency)
    console.log(chargeItem, 'chargeItem')
    result.push(chargeItem)
    return result
  }, [])
  entity.invoiceInfo = entity.payTo.defaultInvoiceInfo ?? null
  recalculateTotal(entity)
  recalculateExchangeRates(entity)
}
...
>
  ...
  <template #action="{ item }">
    ...
    <template v-if="item.status === EbitNoteStatus.Pending && shipmentEditable">
      <v-btn
        :title="$gettext('edit')"
...
<AppTableInline
  ref="tableInvoice"
  :headers="headersInvoiceCredit()"
  :items="shipment.ebitNotes
    .filter(e => e.type === EbitNoteType.InvoiceCredit)
    .sort((a,b) => b.id - a.id)
  "
>
...
<AppForm
  :layout="layout"
  :entityName="$gettext('Credit Note')"
