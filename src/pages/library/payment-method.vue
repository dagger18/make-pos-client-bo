<script setup>
import { filterConfigs, headers } from '@/config/tables/library/PaymentMethod';
import EntityService from '@/services/library/PaymentMethodService';
import { useAppStore } from '@/stores/appStore';
import PaymentMethodForm from '@/views/library/PaymentMethodForm.vue';
import { useLibraryHeader } from '@/composables/useLibraryHeader';
import { useGettext } from 'vue3-gettext';
definePage({
  meta: { action: 'GET', subject: 'PaymentMethod', navActiveLink: 'library-package-type-transport-type' },
})
...
const form = ref(null)
const buttons = computed(() => [{ text: $gettext('Add Payment Method'), func: form.value?.setEntity }])
