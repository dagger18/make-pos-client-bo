<script setup>
import { filterConfigs, headers } from '@/config/tables/library/Incoterm';
import EntityService from '@/services/library/IncotermService';
import { useAppStore } from '@/stores/appStore';
import IncotermForm from '@/views/library/IncotermForm.vue';
import { useLibraryHeader } from '@/composables/useLibraryHeader';
import { useGettext } from 'vue3-gettext';
definePage({
  meta: { action: 'GET', subject: 'Incoterm', navActiveLink: 'library-package-type-transport-type' },
})
...
const form = ref(null)
const buttons = computed(() => [{ text: $gettext('Add Incoterm'), func: form.value?.setEntity }])
