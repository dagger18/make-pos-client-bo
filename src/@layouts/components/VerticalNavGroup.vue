...
const allChildrenLocked = computed(() =>
  isOrgOwner.value &&
  props.item.children?.length > 0 &&
  props.item.children.every(child => child.disabled === true)
)

function handleGroupClick() {
  if (allChildrenLocked.value) {
    openUpgradeDialog()
    return
  }
...
<template>
  <li
    v-if="canViewNavMenuGroup(item) && (isOrgOwner || !props.item.children?.every(c => c.disabled))"
    class="nav-group"
    :class="[
      {
  ...
  >
    <div
      ...
      :style="allChildrenLocked ? 'cursor: pointer' : undefined"
