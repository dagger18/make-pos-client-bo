...
compAttrs.config.parseDate = (datestr, format) => {

  return dateAdapter.parse(datestr, format).toDate();
}
compAttrs.config.formatDate = (date, format, locale) => {
  return dateAdapter.formatByString(date instanceof Date ? dateAdapter.parse(date) : date, format);
}
...
const { fieldRef: errFieldRef, errorMessage } = useFieldTooltipError()
watchEffect(() => { errFieldRef.value = inputRef.value })
</script>
          ...
          <template #default="{ props: vFieldProps }">
            <div v-bind="vFieldProps">
              ...
              <FlatPickr
                ...
                class="flat-picker-custom-style ps-10 ms-n10"
...
.flatpickr-current-month {
  ...
  .flatpickr-monthDropdown-months span {
    display: none;
  }
  ...
  .numInputWrapper {
    span {
      ...
      &.arrowUp::after {
        border-block-end-color: $heading-color;
      }

      &.arrowDown::after {
        border-block-start-color: $heading-color;
      }
    }
  }
}
