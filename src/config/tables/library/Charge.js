import { slugToValue } from '@/config/enums/TransportType';
import { enums as UserStatus } from '@/config/enums/UserStatus';
import { printDateTime, transformDateFilter } from '@/services/CommonService';
export const filterConfigs = (route, appStore) => {
  const transportType = slugToValue(route.params.transportType)
  return [
    {
      title: $gettext('Name'),
      value: 'name',
      type: 'text'
    },
    {
      title: $gettext('Calculation Type'),
      value: 'calculationType',
      type: 'select',
      items: appStore.getList('calculationTypes').filter(({ transportTypes }) => transportTypes.includes(transportType)),
      itemTitle: 'name',
      itemValue: 'id',
    },
    {
      title: $gettext('Custom Calculation Type'),
      value: 'customCalcType',
      type: 'text'
    },
    {
      title: $gettext('Custom Code'),
      value: 'customCode',
      type: 'text'
    },
    {
      title: $gettext('Description'),
      value: 'description',
      type: 'text'
    },
    {
      title: $gettext('Created By'),
      value: 'createdBy',
      type: 'select',
      apiEndpoint: 'UserService',
      searchOnProperties: ['firstName', 'lastName'],
      callParams: { filter_status: UserStatus.ACTIVE },
      itemTitle: 'fullName',
      itemValue: 'id',
      placeholder: $gettext('Select user') + '...'
    },
    {
      title: $gettext('Created On'),
      value: 'createdDate',
      type: 'date',
      preSubmitCallback: transformDateFilter,
      toUTCDateOnPick: false,
      toDateOnMounted: false
    }
  ]
}

export const headers = () => {
  const route = useRoute()
  return [
    { 
      key: 'name', 
      text: $gettext('Name')
    },
    route.params.chargeType === 'custom' ? { 
      key: 'customChargeType', 
      text: $gettext('Type'),
      renderObject (item) {
        return item.customChargeType ?  item.customChargeType.name : ''
      },
    } : null ,
    { key: 'calculationType', text: $gettext('Calculation Type'),
      renderObject (item) {
        return item.calculationType ?  item.calculationType.name : ''
      }
    },
    { key: 'customCalcType', text: $gettext('Custom Calc Type')},
    { key: 'customCode', text: $gettext('Custom Code')},
    { key: 'description', text: $gettext('Description')},
    { key: 'debitTax', text: $gettext('Tax'),
      renderObject (item) {
        return item.debitTax ?  (item.debitTax + '%') : ''
      }
    },
    { 
      key: 'createdBy', 
      text: $gettext('Created By'),
      sortable: false,
      renderAvatar: true,
      headerClass: 'text-center',
      bodyClass: 'text-center'
    },
    { 
      key: 'createdDate', 
      text: $gettext('Created On'),
      renderObject (item) {
        return item.createdDate ?  printDateTime(item.createdDate) : ''
      },
      bodyClass: 'pe-4'
    },
    { 
      key: 'id', 
      text: $gettext('Action'), 
      sortable: false, 
      renderSlot: 'action', 
      bodyClass: 'px-0',
      headerClass: 'text-end pe-4' 
    },
  ].filter(item => item)
}